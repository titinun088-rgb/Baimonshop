...
/**
 * Slip2Go QR Code Generation Proxy
...
async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    const response = await fetch(`${apiUrl}/api/api-qrcode`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      console.error('❌ Slip2Go QR Code Error:', data);
      return res.status(response.status).json({
        success: false,
        error: data.message || 'QR code generation failed'
      });
    }
    ...
    console.error('❌ Slip2Go QR Proxy Error:', error);
    return res.status(500).json({ 
      success: false, 
