    ...
    <Layout>
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl font-bold">แอปพรีเมียม</h1>
          ...
          <Card className="bg-yellow-50 border-yellow-200">
            ...
            <CardContent>
              <div className="space-y-2 text-sm">
                <p><strong>User Role:</strong> {userData?.role || 'undefined'}</p>
          ...
          <Tabs defaultValue="products" className="space-y-4">
            <TabsList>
              ...
              <TabsTrigger value="game-products">สินค้าเติมเกม</TabsTrigger>
              <TabsTrigger value="history">ประวัติการซื้อ</TabsTrigger>
              <TabsTrigger value="preorder">ประวัติพรีออเดอร์</TabsTrigger>
            </TabsList>
            ...
            {/* Game Products Tab */}
            <TabsContent value="game-products" className="space-y-4">
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Package className="h-5 w-5" />
                    รายการสินค้าเติมเกม ({filteredGameProducts.length} รายการ)
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {/* Filters */}
                  <div className="space-y-4 mb-6">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                      <div>
                        <label className="text-sm font-medium">ค้นหาสินค้าเติมเกม</label>
                        <Input
                          placeholder="ค้นหาตามหมวดหมู่หรือข้อมูลสินค้า"
                          value={searchQuery}
                          onChange={(e) => setSearchQuery(e.target.value)}
                        />
                      </div>
                      ...
                      <div>
                        ...
                        >
                          <option value="name">หมวดหมู่</option>
                          <option value="price">ราคา</option>
                        </select>
                      </div>
                      <div className="flex items-end">
                  ...
                  {filteredGameProducts.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      {searchQuery || maxPrice ? 
                        "ไม่พบสินค้าเติมเกมตามเงื่อนไขที่กำหนด" : 
                        "ยังไม่มีข้อมูลสินค้าเติมเกม"
                      }
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {filteredGameProducts.map((product) => (
                        <Card key={product.id} className="overflow-hidden">
                          <div className="aspect-square overflow-hidden">
                            <img
                              src={product.img}
                          ...
                          <CardContent className="p-4">
                            ...
                            <div className="space-y-2 mb-4">
                              <div className="flex justify-between items-center">
                                <span className="text-sm text-muted-foreground">ราคาแนะนำ:</span>
                                <span className="font-semibold text-lg text-green-600">{product.recommendedPrice} บาท</span>
                              </div>
                              <div className="flex justify-between items-center">
                                <span className="text-sm text-muted-foreground">ราคาขาย:</span>
                                <span className="font-semibold text-green-600">
                                  {('recommendedPrice' in product && product.recommendedPrice) ? product.recommendedPrice : product.price} บาท
                                </span>
                              </div>
                              
                              {/* แสดงราคาต้นทุนสำหรับแอดมิน */}
                              {isAdmin && (
                                <div className="flex justify-between items-center">
                                  <span className="text-sm text-muted-foreground">ราคาต้นทุน:</span>
                                  <span className="font-semibold text-blue-600">{product.price} บาท</span>
                                </div>
                              )}
                            </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    ...
                    ประวัติการซื้อ ({filteredHistory.length} รายการ)
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {/* History Filters */}
                  <div className="space-y-4 mb-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        <label className="text-sm font-medium">ค้นหาประวัติ</label>
                        <Input
                          placeholder="ค้นหาตามชื่อสินค้าหรือรหัสอ้างอิง"
                  ...
                  {filteredHistory.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      {historySearchQuery || historyStatusFilter !== "all" ? 
                        "ไม่พบประวัติตามเงื่อนไขที่กำหนด" : 
            ...
            {/* Preorder History Tab */}
            <TabsContent value="preorder" className="space-y-4">
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Clock className="h-5 w-5" />
                    ประวัติพรีออเดอร์ ({filteredPreorderHistory.length} รายการ)
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {/* Preorder History Filters */}
                  <div className="space-y-4 mb-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        <label className="text-sm font-medium">ค้นหาประวัติพรีออเดอร์</label>
                        <Input
                          placeholder="ค้นหาตามชื่อสินค้าหรือรหัสอ้างอิง"
                      ...
                      <div>
                        <label className="text-sm font-medium">สถานะ</label>
                        <Select value={preorderStatusFilter} onValueChange={setPreorderStatusFilter}>
                          <SelectTrigger>
                            <SelectValue placeholder="เลือกสถานะ" />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <div className="flex items-end">
                        <Button
                          onClick={handleResetPreorderFilters}
                          variant="outline"
                          className="w-full"
                        >
                          <Filter className="mr-2 h-4 w-4" />
                  ...
                  {/* Preorder History Table */}
                  {filteredPreorderHistory.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      {preorderApiStatus === 'unavailable' ? (
                        <div className="space-y-2">
                          <AlertTriangle className="h-12 w-12 mx-auto text-yellow-500" />
                          <p className="font-semibold">ระบบพรีออเดอร์ไม่พร้อมใช้งาน</p>
                          <p className="text-sm">ไม่สามารถโหลดประวัติพรีออเดอร์ได้ในขณะนี้</p>
                        </div>
                      ) : preorderSearchQuery || preorderStatusFilter !== "all" ? (
                        "ไม่พบประวัติพรีออเดอร์ตามเงื่อนไขที่กำหนด"
                      ) : (
                        "ยังไม่มีประวัติพรีออเดอร์"
                      )}
                    </div>
                  ) : (
                    <div className="rounded-lg border border-border overflow-hidden">
                      <Table>
                        ...
                        <TableBody>
                          {filteredPreorderHistory.map((item) => (
                            <TableRow key={item.id}>
                              <TableCell>
                                <img
                                  src={item.img}
